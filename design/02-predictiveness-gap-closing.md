# pattern-boost — Predictiveness & Closing the Gap to Unconstrained GBMs

> 2026-06-21. Companion to [`01-brainstorm.md`](01-brainstorm.md). Produced by a gap-closing analysis team (HP-ensemble deep-dive + ranked gap-closer synthesis + closability reality-check + 2023–26 frontier scan), then synthesized. Answers two questions: **(1)** is a hyperparameter-diverse ensemble of pattern-boost models worth it, and how; **(2)** what components close the predictiveness gap to unconstrained XGBoost/LightGBM/CatBoost *while staying exactly decomposable into ≤3rd-order tables*.
>
...
# pattern-boost: Closing the Predictiveness Gap While Staying Exactly Decomposable
...
- **Oblivious vs leaf-wise penalty ≈ 0.** CatBoost (oblivious) and LightGBM (leaf-wise) sit within a few rungs on TabArena; CatBoost ranks *first* untuned. Obliviousness itself costs essentially nothing at the ensemble level — pattern-boost's structural penalty is the *order cap*, not symmetric growth.
...
Items 1–5 are the v1 spine that gets you to "beat EBM, near-parity on most data." Item 6 is the highest-upside addition for chasing the black boxes. Item 7 squeezes the last 0.5–1.5% for users who can pay K×. Throughout, the honest disclosure stands: none of this lifts the order-3 *bias* ceiling — on genuine ≥4-way data (Higgs/Year-likes) pattern-boost is the wrong tool, and that minority is the irreducible cost of exact ≤3rd-order decomposition.
