...
- *Decomposability:* the **soft model itself is not exactly decomposable** (soft routing = fractional cell membership, DenseNet stacking explodes order — research/06-2 §111,117). You'd have to *distill* it into the hard constant-cell model, at which point you're back to pattern-boost as the student and the NN is just a teacher.
...
The architecture is **right in its bones** and should not be replaced: boosting-family (not a from-scratch GA3M, not a soft NN), oblivious symmetric trees, constant cells, purified fANOVA, the typed exactness firewall. Every genuinely different design I steelmanned either *is* pattern-boost with a relaxed constraint (A), survives only as a *teacher/detector* the spec already uses (B), or is an additive schedule the spec should bolt on (C).
