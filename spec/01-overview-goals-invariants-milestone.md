...
1. **MAXIMUM PREDICTIVENESS.** As accurate as the constraint allows, using every gap-closer in the playbook (`design/02`): Newton summed-gain splits with exact leaf weights, the full order-3 budget with exact 3D tables, the heredity+FAST+Sobol admission funnel under *joint* boosting, Fisher sorted-ordinal target statistics, log-link deviance objectives, and — the highest-upside new lever — CatBoost teacher-distillation. Accuracy is chased aggressively, but **never by bending I1/I2** (§1.4); any technique that would (linear leaves, continuous-TS axes, combination CTRs, >3-order base margins) is forbidden on the exact path or quarantined behind the firewall (§3, §1.5).
...
- **Completeness, not truncation.** Unlike EBM/GA2M (which *cap* at order 2) or post-hoc fANOVA on a black box (which *truncates and approximates* an unbounded expansion), pattern-boost's decomposition is provably complete: there are no order-≥4 terms to drop. This is why the tables *are* the model rather than a summary of it.
...
**In scope (the library core).** The depth-3 oblivious boosting engine (§06); the `Loss` trait and its v1 implementors `SquaredError`, `Logistic`, `Poisson`, `Gamma`, `Tweedie { rho }` with exact (g,h), exposure offsets, and deviance early-stopping (§05); binning and the shared grid (§03); leakage-free Fisher sorted-ordinal categoricals (§04); interaction selection, monotone and `max_interaction_order ∈ {1,2,3}` constraints (§07); the purification/fANOVA explainability engine and exact interventional SHAP/Faith-Shap ≤order-3 (§08); distillation and self-ensembling (§09); serialization and inference (§10); the Python API (§12). Pricing **objectives** are in scope precisely because they are loss functions; the **table decomposition** is in scope because it is the explainability output.
...
> **v1 milestone.** On the **TabArena** benchmark suite, every fitted pattern-boost model **(a) beats EBM / GA2M** on held-out deviance/logloss, and **(b) comes within striking distance of unconstrained XGBoost / LightGBM / CatBoost**, **while (c) every model stays bit-exactly decomposable** into ≤3rd-order tables (all five I2 checks pass, `ExactnessMode::Exact`).
...
- **Beat EBM essentially everywhere.** pattern-boost has strictly more capacity (exact order-3 vs EBM's order-2 cap), full Newton summed-gain splits (vs EBM's tiny-LR stumps), and CatBoost-grade categoricals (Fisher TS). EBM trails LightGBM by ~160 Elo on TabArena's 51 datasets — a clear, beatable deficit.
