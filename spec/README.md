...
- **Decision:** `#![forbid(unsafe_code)]` by default; SIMD via safe `multiversion`/`pulp`/`wide` wrappers; any `unsafe` is encapsulated, `// SAFETY:`-proven, tested, Miri-run. **Rationale:** safety-first engineering standard. **Owner:** §1/§11.
...
- **Decision:** MSRV **1.64** (manylinux2014 floor), pinned; `cargo deny` is `[GATE]`. **Rationale:** broad portability, audited deps. **Owner:** §1/§13.
