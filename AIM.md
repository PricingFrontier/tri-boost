...
1. **Exact decomposition first — the v1 "green-spine alpha".** v1 proves the load-bearing thesis: depth-3 oblivious Newton histogram boosting on **SquaredError**, accumulated and purified into constant + 1D + 2D + 3D tables, with the **lossless-equivalence property** holding as a *tested invariant* (reconstructed tables == tree ensemble, to floating-point). The exact-decomposition spine is the v1 exit criterion — it is **not** gated on any benchmark result. **Predictive parity — the EBM / TabArena milestone — is the v1.5 release gate, not v1:** once the v1.5 accuracy levers land (Fisher categorical target statistics, quantized histograms, MVS, distillation, ensemble, the full objective set), the model must, on the **TabArena** benchmark suite, **beat EBM / GA2M** and come within striking distance of unconstrained XGBoost / LightGBM / CatBoost, while *every* model stays exactly decomposable into ≤3rd-order tables. Competitive predictiveness *with* exact explainability is the whole thesis — but the v1 spine proves the decomposition is exact before the v1.5 gate proves it is accurate. (We use TabArena as our own yardstick; we do not ship benchmarking tooling.)
2. A trained model **exports a small, human-readable** set of tables / rating relativities that an actuary or analyst can read directly.
